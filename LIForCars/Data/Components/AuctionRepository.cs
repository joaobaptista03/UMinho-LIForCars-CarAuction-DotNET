...
namespace LIForCars.Data.Components
{
    public class AuctionRepository : IAuctionRepository
    {
        ...
        public bool UpdateAuthorizationStatus(int auctionId)
        {
            try
            {
                var auction = _context.Auction.Find(auctionId);

                if (auction != null)
                {
                    auction.Autorized = true;
                    _context.SaveChanges();
                    return true;
                }
        ...
        public async Task<(IEnumerable<Auction> auctions, int totalCount)> GetCurrentAuctionsAsync(int page, int pageSize, string orderBy, string filterBy)
        {
            ...
            IQueryable<Auction> filtered = new List<Auction>().AsQueryable();
            var totalCount = 0;
            if (orderBy=="RemainingTimeDescending") {
                if (filterBy=="") {
                    filtered = query.OrderByDescending(a => a.EndDateTime);

                    totalCount = filtered.Count();

                    auctions = await filtered.Skip((page - 1) * pageSize)
                                .Take(pageSize)
                                .ToListAsync();
                } else {
        ...
        public async Task<(IEnumerable<Auction> auctions, int totalCount)> GetAllWaittingAuctionsAsync(int page, int pageSize) {
            var query = _context.Auction
                                      .Include(async => async.Car)
                                      .Where(a => a.Autorized == false)
                                      .OrderBy(async => async.InitDateTime);
                                      
            var totalCount = query.Count();
                                      
                                      
            var auctions = await query.Skip((page - 1) * pageSize)
                                      .Take(pageSize)
                                      .ToListAsync();
